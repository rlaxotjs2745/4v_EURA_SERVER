...
BEGIN
	...
	SELECT COUNT(1) INTO @CNT FROM TB_MEETING WHERE IDX_USER=_idx_user AND DELETE_STAT=0 AND IS_FINISH=0 AND MT_STATUS IN (0,1) AND IDX_MEETING!=_idx_meeting
...
BEGIN
	...
	IF _idx_meeting IS NOT NULL THEN
		...
		IF @MTSTARTDT != _mt_start_dt AND @MTENDDT != _mt_end_dt THEN
			SELECT COUNT(1) INTO @CNT FROM TB_MEETING WHERE IDX_USER=_idx_user AND DELETE_STAT=0 AND IS_FINISH=0 AND MT_STATUS IN (0,1)
