...
CREATE TABLE `TB_MAIL_MEET` (
  `IDX_MAIL_RESERVED` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `IDX_USER` int(10) unsigned DEFAULT NULL COMMENT '발송회원 ',
  `RECEIVER` varchar(400) DEFAULT NULL COMMENT '메일주소(회원메일) ',
  `TITLE` varchar(1000) DEFAULT NULL COMMENT '메일 제목 ',
  `CONTENT` longtext DEFAULT NULL COMMENT '메일 내용 ',
  `MAIL_TYPE` tinyint(1) DEFAULT NULL COMMENT '0:뭘보낸겨, 1:시작, 2:30분전, 3:취소, 4:공개, 5:인증, 6:임시비번',
  `IDX_MEETING_USER_JOIN` int(11) DEFAULT NULL,
  `REG_DT` datetime DEFAULT current_timestamp(),
  PRIMARY KEY (`IDX_MAIL_RESERVED`)
) ENGINE=MyISAM DEFAULT CHARSET=utf8mb4 COMMENT='미팅 메일 발송 정보';

CREATE TABLE `TB_MEETING_ANALYSIS_PERSONAL` (
  `IDX_MEETING_ANALYSIS_PERSONAL` int(11) unsigned NOT NULL AUTO_INCREMENT,
  `IDX_MEETING` int(11) NOT NULL,
  `IDX_MEETING_USER_JOIN` int(11) NOT NULL,
  `TOTAL_GOOD` tinyint(11) DEFAULT 0,
  `TOTAL_BAD` tinyint(11) DEFAULT 0,
  PRIMARY KEY (`IDX_MEETING_ANALYSIS_PERSONAL_BAR`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='개인 몰이도 데이터';

CREATE TABLE `TB_MEETING_ANALYSIS_PERSONAL_BAR` (
  `IDX_MEETING_ANALYSIS_PERSONAL_BAR` int(11) unsigned NOT NULL AUTO_INCREMENT,
  `IDX_MEETING` int(11) NOT NULL,
  `MOVIE_NO` int(11) DEFAULT 0,
  `IDX_MEETING_USER_JOIN` int(11) NOT NULL,
  `GOOD` tinyint(11) DEFAULT 0,
  `BAD` tinyint(11) DEFAULT 0,
  `LVL` varchar(20) DEFAULT NULL,
  PRIMARY KEY (`IDX_MEETING_ANALYSIS_PERSONAL_BAR`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='개인 그래프용 데이터';

CREATE TABLE `TB_MEETING_ANALYSIS_TOTAL` (
  `IDX_MEETING_ANALYSIS_TOTAL` int(11) unsigned NOT NULL AUTO_INCREMENT,
  `IDX_MEETING` int(11) NOT NULL,
  `TOTAL_GOOD` tinyint(11) DEFAULT 0,
  `TOTAL_BAD` tinyint(11) DEFAULT 0,
  PRIMARY KEY (`IDX_MEETING_ANALYSIS_TOTAL`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='전체 몰입도 데이터(반원 그래프)';

CREATE TABLE `TB_MEETING_ANALYSIS_TOTAL_BAR` (
  `IDX_MEETING_ANALYSIS_TOTAL_BAR` int(11) unsigned NOT NULL AUTO_INCREMENT,
  `IDX_MEETING` int(11) NOT NULL,
  `MOVIE_NO` int(11) DEFAULT 0,
  `GOOD` tinyint(11) DEFAULT 0,
  `BAD` tinyint(11) DEFAULT 0,
  `LVL` varchar(20) DEFAULT NULL,
  PRIMARY KEY (`IDX_MEETING_ANALYSIS_TOTAL_BAR`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='전체 그래프용 데이터';